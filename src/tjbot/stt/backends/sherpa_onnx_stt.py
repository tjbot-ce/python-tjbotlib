...
class SherpaONNXSTTEngine(STTEngine):
    ...
    def initialize(self, microphone_rate: int, microphone_channels: int) -> None:
        ...
        try:
            ...
            if vad_config and self._model_kind and self._model_kind.startswith('offline'):
                ...
                if vad_enabled and vad_model_key:
                    ...
                    self._vad_path = str(vad_cache_dir / vad_info.folder / vad_info.required[0])
    ...
    def transcribe(self, audio_stream: Iterator[bytes], options: Optional[STTRequestOptions] = None) -> str:
        ...
        try:
            if self._model_kind in ('streaming-zipformer', 'streaming-paraformer'):
                transcript = self._transcribe_online(audio_stream, on_partial_result, on_final_result, abort_signal)
            elif self._should_use_vad():
                transcript = self._transcribe_offline_with_vad(audio_stream, on_partial_result, on_final_result, abort_signal)
            else:
                transcript = self._transcribe_offline_energy(audio_stream, on_partial_result, on_final_result, abort_signal)

            if not transcript.strip():
                raise TJBotError('STT: No speech could be recognized', code='stt.no-speech')
            return transcript
        except Exception as e:
    ...
    def _transcribe_offline_with_vad(
        ...
        vad = self._create_silero_vad(self._vad_path)
        sample_rate = 16000
