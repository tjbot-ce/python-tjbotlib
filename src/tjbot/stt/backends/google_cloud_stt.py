...
def _to_google_cloud_recognition_error(error: Exception, recognizer_path: str) -> TJBotError:
...
class GoogleCloudSTTEngine(STTEngine):
    ...
    def initialize(self, microphone_rate: int, microphone_channels: int) -> None:
        ...
        try:
            ...
            logger.info('Google Cloud STT v2 initialized (model=%s, region=%s)', model, region)
    ...
    def transcribe(self, audio_stream: Iterator[bytes], options: Optional[STTRequestOptions] = None) -> str:
        ...
        language_code = (cfg.language_code or '').strip() if cfg else ''
        region = (cfg.region or '').strip() if cfg else ''
        enable_automatic_punctuation: bool = cfg.enable_automatic_punctuation if (cfg and cfg.enable_automatic_punctuation is not None) else True
        profanity_filter: bool = cfg.profanity_filter if (cfg and cfg.profanity_filter is not None) else True
        interim_results: bool = cfg.interim_results if (cfg and cfg.interim_results is not None) else True
        ...
        logger.debug(
            ...
            model, language_code, recognizer_path,
        )
        ...
        def _request_generator() -> Iterator[cs.StreamingRecognizeRequest]:
            ...
            for chunk in audio_stream:
                if self._is_abort_signal_set(abort_signal):
                    raise TJBotError('Google Cloud STT transcription aborted', code='stt.aborted')
        ...
        try:
            ...
            if not transcript:
                raise TJBotError('Google Cloud STT: No speech could be recognized', code='stt.no-speech')
            return transcript
