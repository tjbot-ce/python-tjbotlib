...
STTModelFlavor = Literal['streaming-zipformer', 'streaming-paraformer', 'offline-whisper', 'offline-moonshine']
...
def infer_local_model_flavor(model_name: Optional[str] = None, model_url: Optional[str] = None) -> STTModelFlavor:
    """
    ...
    is_zipformer = bool(re.search(r'zipformer|transducer|streaming-zipformer', haystack))
...
def infer_stt_mode(listen_config: ListenConfig) -> STTModelType:
    ...
    if backend_type == 'ibm-watson-stt':
        cfg = backend.ibm_watson_stt if backend else None
        if cfg and cfg.interim_results:
            return 'streaming'
        return 'offline'

    if backend_type == 'google-cloud-stt':
        cfg = backend.google_cloud_stt if backend else None
        if cfg and cfg.interim_results:
            return 'streaming'
        return 'offline'
    ...
    if backend_type == 'local':
        cfg = backend.local if backend else None
        model_name = cfg.model if cfg else None
        model_url = cfg.model_url if cfg else None
