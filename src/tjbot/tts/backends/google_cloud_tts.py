...
class GoogleCloudTTSEngine(TTSEngine):
	...
	async def initialize(self) -> None:
		...
		language_code = self.backend_config.language_code if self.backend_config else None
		if not voice:
			raise TJBotError(
	...
	async def synthesize(self, text: str) -> bytes:
		if not self.client:
			raise TJBotError('Google Cloud TTS client not initialized. Call initialize() first.')

		self.validate_text(text)

		language_code = self.backend_config.language_code if self.backend_config else 'en-US'
		voice_name = self.backend_config.voice if self.backend_config else None
		if not voice_name:
			raise TJBotError('Google Cloud TTS voice not specified. Provide voice in speak config.')
