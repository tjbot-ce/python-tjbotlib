...
class AzureVisionEngine(VisionEngine):
    ...
    def _ensure_client(self):
        if not self.client:
            raise TJBotError('Azure Vision client not initialized. Call initialize() first.')
    ...
    def _object_detection_threshold(self) -> float:
        if not self.config or self.config.object_detection_confidence is None:
            raise TJBotError('Object detection confidence threshold is not configured for Azure Vision engine')
    ...
    def _classification_threshold(self) -> float:
        if not self.config or self.config.image_classification_confidence is None:
            raise TJBotError('Image classification confidence threshold is not configured for Azure Vision engine')
    ...
    async def detect_objects(self, image: ImageInput) -> List[ObjectDetectionResult]:
        ...
        try:
            result = self.client.analyze_image_in_stream(self._read_image(image), visual_features=['Objects'])
                ...
                output.append(
                    {
                        ...
                        'bbox': (float(rect.x), float(rect.y), float(rect.w), float(rect.h)),
                    }
                )
    ...
    async def classify_image(self, image: ImageInput) -> List[ImageClassificationResult]:
        ...
        try:
            result = self.client.analyze_image_in_stream(self._read_image(image), visual_features=['Tags'])
            output = [
                {
    ...
    async def describe_image(self, image: ImageInput) -> ImageDescriptionResult:
        ...
        try:
            result = self.client.analyze_image_in_stream(self._read_image(image), visual_features=['Description'])
            captions = (result.description.captions if result.description else None) or []
            if not captions:
