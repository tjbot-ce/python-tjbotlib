...
class GoogleCloudVisionEngine(VisionEngine):
    ...
    def _ensure_client(self):
        if not self.client:
            raise TJBotError(
                "Google Cloud Vision client not initialized. Call initialize() first."
            )
    ...
    def detect_objects(self, image: ImageInput) -> List[ObjectDetectionResult]:
        ...
        try:
            ...
            if response.error.message:
                raise TJBotError(
                    f"Google Cloud Vision API error during object detection: {response.error.message}"
                )
            ...
            if isinstance(error, TJBotError):
                raise
            raise TJBotError(
                f"Google Cloud Vision API error during object detection: {error}"
            )
    ...
    def classify_image(self, image: ImageInput) -> List[ImageClassificationResult]:
        ...
        try:
            ...
            if isinstance(error, TJBotError):
                raise
            raise TJBotError(
                f"Google Cloud Vision API error during classification: {error}"
            )
    ...
    def detect_faces(self, image: ImageInput) -> FaceDetectionResult:
        ...
        try:
            ...
            if isinstance(error, TJBotError):
                raise
            raise TJBotError(
                f"Google Cloud Vision API error during face detection: {error}"
            )
