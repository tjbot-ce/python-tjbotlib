...
def run_test() -> None:
    ...
    backend_label = next((b['label'] for b in BACKENDS if b['id'] == selected_backend), 'Unknown')
    ...
    if can_annotate and task in ('detectObjects', 'detectFaces') and result and isinstance(result, dict) and isinstance(result.get('metadata'), list):
...
def prompt_task_choice(selected_backend: str) -> str:
    ...
    classification_label = classification_models[0].label or classification_models[0].key
    face_detection_label = face_detection_models[0].label or face_detection_models[0].key

    tasks: List[Dict[str, str]] = [
        {
            'name': f'Object detection ({detection_label})' if selected_backend == 'local' else 'Object detection',
            'value': 'detectObjects',
        },
        {
            'name': f'Image classification ({classification_label})' if selected_backend == 'local' else 'Image classification',
            'value': 'classifyImage',
        },
    ]
    ...
    if selected_backend != 'azure-vision':
        tasks.append({
            'name': f'Face detection ({face_detection_label})' if selected_backend == 'local' else 'Face detection',
            'value': 'detectFaces',
        })
    ...
    with Image.open(img_path) as image:
        ...
        for item in result.get('metadata', []):
            ...
            if confidence is not None:
                draw.text((x1 + 5, max(0, y1 - 18)), f'{confidence * 100:.1f}%', fill=color)
