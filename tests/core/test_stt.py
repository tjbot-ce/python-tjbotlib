...
def test_load_sherpa_module_returns_imported_module(monkeypatch):
    ...
    monkeypatch.setattr(
        "tjbot.utils.sherpa_runtime._find_sherpa_bundled_lib_dir", lambda: None
    )
...
def test_load_sherpa_module_preloads_when_lib_dir_found(monkeypatch, tmp_path):
    """load_sherpa_onnx_module() calls _preload_sherpa_bundled_libs when lib_dir exists."""
    fake_module = types.ModuleType("sherpa_onnx")
    fake_lib_dir = tmp_path / "lib"
    fake_lib_dir.mkdir()

    monkeypatch.setattr(
        "tjbot.utils.sherpa_runtime._find_sherpa_bundled_lib_dir",
        lambda: fake_lib_dir,
    )
    preload_calls: list = []
    monkeypatch.setattr(
        "tjbot.utils.sherpa_runtime._preload_sherpa_bundled_libs",
        lambda d: preload_calls.append(d),
    )
    monkeypatch.setattr("importlib.import_module", lambda name: fake_module)

    load_sherpa_onnx_module()
    assert preload_calls == [fake_lib_dir]


def test_load_sherpa_module_skips_preload_when_no_lib_dir(monkeypatch):
    """load_sherpa_onnx_module() skips preloading when lib_dir is not found."""
    fake_module = types.ModuleType("sherpa_onnx")
    monkeypatch.setattr(
        "tjbot.utils.sherpa_runtime._find_sherpa_bundled_lib_dir", lambda: None
    )
...
def test_preload_sherpa_bundled_libs_loads_ort_then_capi(tmp_path):
    """_preload_sherpa_bundled_libs loads libonnxruntime then libsherpa-onnx-c-api."""
    lib_dir = tmp_path / "lib"
    lib_dir.mkdir()
    ort = lib_dir / "libonnxruntime.so.1.24.4"
    capi = lib_dir / "libsherpa-onnx-c-api.so"
    ort.write_bytes(b"")
    capi.write_bytes(b"")

    loaded: list = []
    with patch.object(
        ctypes.cdll, "LoadLibrary", side_effect=lambda p: loaded.append(p)
    ):
        _preload_sherpa_bundled_libs(lib_dir)

    assert any("libonnxruntime" in p for p in loaded)
    assert any("libsherpa-onnx-c-api" in p for p in loaded)
    # onnxruntime must be loaded before the capi lib
    ort_idx = next(i for i, p in enumerate(loaded) if "libonnxruntime" in p)
    capi_idx = next(i for i, p in enumerate(loaded) if "libsherpa-onnx-c-api" in p)
    assert ort_idx < capi_idx


def test_preload_sherpa_bundled_libs_skips_missing_patterns(tmp_path):
    """_preload_sherpa_bundled_libs gracefully handles an empty lib dir."""
    lib_dir = tmp_path / "lib"
    lib_dir.mkdir()
...
def test_preload_sherpa_bundled_libs_continues_on_oserror(tmp_path):
    """_preload_sherpa_bundled_libs skips a lib that fails to load without raising."""
    lib_dir = tmp_path / "lib"
    lib_dir.mkdir()
    bad = lib_dir / "libonnxruntime.so.1.0.0"
    bad.write_bytes(b"")

    with patch.object(ctypes.cdll, "LoadLibrary", side_effect=OSError("cannot open")):
        # should not raise
        _preload_sherpa_bundled_libs(lib_dir)
...
def test_detect_objects_uses_mocked_runtime_and_postprocess(monkeypatch):
    ...
    result = engine.detect_objects(b"image-bytes")
...
def test_classify_image_uses_mocked_runtime_and_postprocess(monkeypatch):
    ...
    result = engine.classify_image(b"image-bytes")
...
def test_detect_faces_uses_mocked_runtime_and_postprocess(monkeypatch):
    ...
    result = engine.detect_faces(b"image-bytes")
